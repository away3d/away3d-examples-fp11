package
{
	...
	[SWF(width="1280", height="768", backgroundColor="0x000000", frameRate="60")]
	public class Advanced_ShallowWaterDemo extends Sprite
	{
		...
		private function initObjects():void {
			...
			_planeSize = planeSegments * gridSpacing;
			_plane = new Mesh( new PlaneGeometry( _planeSize, _planeSize, planeSegments, planeSegments ), _liquidMaterial );
			_plane.rotationX = 90;
			_plane.x -= _planeSize / 2;
			_plane.z -= _planeSize / 2;
			...
			var poolHOffset:Number = _planeSize / 2 + poolThickness / 2;
			var left:Mesh = new Mesh( new CubeGeometry( poolThickness, poolHeight, _planeSize + poolThickness * 2 ), poolMaterial );
			left.x = -poolHOffset;
			left.y = poolVOffset;
			scene.addChild( left );
			var right:Mesh = new Mesh( new CubeGeometry( poolThickness, poolHeight, _planeSize + poolThickness * 2 ), poolMaterial );
			right.x = poolHOffset;
			right.y = poolVOffset;
			scene.addChild( right );
			var back:Mesh = new Mesh( new CubeGeometry( _planeSize, poolHeight, poolThickness ), poolMaterial );
			back.z = poolHOffset;
			back.y = poolVOffset;
			scene.addChild( back );
			var front:Mesh = new Mesh( new CubeGeometry( _planeSize, poolHeight, poolThickness ), poolMaterial );
			front.z = -poolHOffset;
			front.y = poolVOffset;
		...
		private function planeMouseMoveHandler( event:MouseEvent3D ):void {
			var x:Number = event.localX / _planeSize;
			var y:Number = event.localY / _planeSize;
			if( mouseBrushLife == 0 )
				_fluidDisturb.disturbBitmapInstant( x, y, -mouseBrushStrength, _mouseBrush.bitmapData );
			else
				_fluidDisturb.disturbBitmapMemory( x, y,
		...
		private function stageKeyDownHandler( event:KeyboardEvent ):void {
			switch( event.keyCode ) {
				case Keyboard.M:
				{
					toggleShading = !_liquidShading;
					break;
				}
				case Keyboard.B:
				{
					var index:uint = brushClips.indexOf( _activeMouseBrushClip );
					index = index + 1 < brushClips.length ? index + 1 : 0;
					activeMouseBrushClip = brushClips[ index ];
					break;
				}
				case Keyboard.NUMBER_1:
				{
					toggleLiquidImage = !_showingLiquidImage;
					break;
				}
				case Keyboard.NUMBER_2:
				{
					toggleLiquidImage1 = !_showingLiquidImage1;
					break;
				}
				case Keyboard.NUMBER_3:
				{
					toggleLiquidImage2 = !_showingLiquidImage2;
					break;
				}
				case Keyboard.R:
				{
